...
namespace GILES
{
	...
	public class pb_SceneCamera : MonoBehaviour
	{
		...
		const string INPUT_MOUSE_X = "Horizontal";
		const string INPUT_MOUSE_Y = "Vertical";
		...
		void LateUpdate()
		{
			...
			if( (Input.mouseScrollDelta.y != 0f || (Input.GetMouseButton(RIGHT_MOUSE) && Input.GetKey(KeyCode.LeftAlt))) && CheckMouseOverGUI())
			{
